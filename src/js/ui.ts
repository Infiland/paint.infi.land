import { TOOL_ORDER, type JoinPayload, type Tool } from "../shared/protocol.js";
...
interface UIElements {
  ...
  smoothingToggle: HTMLButtonElement;
  themeToggle: HTMLButtonElement;
  zoomLevel: HTMLElement;
}

interface UIActions {
  joinSession: (payload: JoinPayload) => void;
  updateColor: (color: string) => void;
  isConnected: () => boolean;
}

interface ViewControlElements {
  zoomInBtn: HTMLButtonElement;
  zoomOutBtn: HTMLButtonElement;
  resetViewBtn: HTMLButtonElement;
}

function syncToolButtons(toolbar: HTMLElement, activeTool: Tool): void {
  for (const button of toolbar.querySelectorAll<HTMLButtonElement>(".tool-btn[data-tool]")) {
...
function applySmoothingPreference(enabled: boolean): void {
...
function syncStatus(elements: UIElements): void {
...
function updateViewScale(nextScale: number): void {
...
export function setupUI(elements: UIElements, actions: UIActions): void {
  ...
  elements.toolbar.addEventListener("click", (event) => {
    ...
    const button = event.target.closest(".tool-btn[data-tool]") as HTMLButtonElement | null;
...
export function setupViewControls({ zoomInBtn, zoomOutBtn, resetViewBtn }: ViewControlElements): void {

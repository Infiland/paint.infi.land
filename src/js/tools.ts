import type { Point, Shape } from "../shared/protocol.js";
import { requestRender } from "./render.js";
import { emitCursor, socket, startDrawInterval, stopDrawInterval } from "./socket.js";
import { clampScale, createLocalShapeId, state, type PointerSnapshot } from "./state.js";
...
function resizeTextEditor(editor: HTMLTextAreaElement): void {
...
function toCanvasPoint(pointer: ScreenPointer): { x: number; y: number } {
  return {
...
function screenToWorld(pointer: ScreenPointer): { x: number; y: number } {
  return {
    x: (pointer.clientX - state.viewOffsetX) / state.viewScale,
    y: (pointer.clientY - state.viewOffsetY) / state.viewScale,
  };
...
function setScaleAroundWorldPoint(newScale: number, worldPoint: { x: number; y: number }, screenPoint: ScreenPointer): void {
...
function commitLocalShape(shape: Shape): void {
...
function clearActiveTextEditor(editor: HTMLTextAreaElement): void {
...
export function commitActiveTextEditor(): void {
...
function discardActiveTextEditor(): void {
  const editor = state.textEditor;
  if (!editor) {
    return;
  }
...
function startTextEditing(screenX: number, screenY: number, worldX: number, worldY: number): void {
...
function deleteShape(shape: Shape): void {
  state.undoStack.push({ type: "delete", shape });
  state.redoStack.length = 0;
  socket.emit("shapeDelete", { id: shape.id });
}

function handleEraseAtPoint(x: number, y: number): void {
...
export function onPointerMove(event: PointerEvent): void {
  ...
  for (const pointerEvent of eventsToProcess) {
    ...
    if (state.tool === "pen") {
      state.pointsBuffer.push([x, y]);
      if (state.selfId) {
        const entry = state.activeStrokes.get(state.selfId) ?? { type: "pen" as const, color: state.color, points: [] as Point[] };
    ...
    if (state.tool === "rect" || state.tool === "circle") {
      const entry = state.activeStrokes.get(state.selfId) ?? { type: state.tool, color: state.color, points: [] as Point[] };
...
export function onWheelZoom(event: WheelEvent): void {
  if (!event.ctrlKey) {
    return;
  }
